    ...
    <View className="space-y-6 pb-12">
      ...
      <View className="mb-4">
        <View className="flex-row items-center gap-2 mb-3">
          ...
          <CustomText className="text-sm font-bold text-orange-500">Hari Ini ({dueToday.length})</CustomText>
        </View>
        {dueToday.length === 0 ? (
           <View className="border-2 border-dashed rounded-2xl p-4 items-center justify-center" style={{ borderColor: `${borderColor}80` }}>
              {/* FIX WARNA: Menggunakan style={{ color }} agar tembus Dark Mode */}
        ...
        <View className="pt-4 border-t mt-4" style={{ borderColor: `${borderColor}80` }}>
          ...
          {completedTodos.map(todo => (
            <TouchableOpacity 
              ...
              // FIX: Gaya kartu compact disinkronkan dengan TaskItem, hijau terang!
              style={{ 
